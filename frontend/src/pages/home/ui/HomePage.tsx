...
import { Building2, Stethoscope, AlertCircle, Activity } from 'lucide-react';
...
export const HomePage = () => {
  ...
  const handleCardClick = (hospitalId: string, hospitalName: string) => {
    // Check custom claims
    const hasAccess = 
      currentUser.customClaims.accessibleModules.includes('all') || 
      currentUser.customClaims.accessibleModules.includes(hospitalId);
      
    const isDenied = currentUser.customClaims.deniedModules?.includes(hospitalId);

    if (!hasAccess || isDenied) {
      setToastMsg(`[${hospitalName}] 접근 권한이 없습니다.`);
      setTimeout(() => setToastMsg(''), 3000);
      return;
    }

    // Navigate to hospital main program
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2 tracking-tight">의료종별 병원 선택</h1>
          <p className="text-slate-500">접속할 병원의 종류를 선택하여 관리 시스템으로 이동하세요.</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {medicalTypes.map((hospital) => {
              ...
              >
                <div className="p-5 flex flex-col items-center justify-center text-center gap-3">
                  <div className="h-12 w-12 rounded-2xl bg-blue-50/80 text-blue-600 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-sm border border-blue-100">
                    {hospital.category === 'Dental' ? <Stethoscope className="h-6 w-6" /> : <Building2 className="h-6 w-6" />}
                  </div>

...
import { Home, LayoutDashboard, Settings, CalendarCheck } from 'lucide-react';

const DeskLayout = () => {
  const navigate = useNavigate();
  
  const menuItems = [
    { name: '대시보드', icon: <LayoutDashboard size={20} />, path: '/desk' },
    { name: '환경설정', icon: <Settings size={20} />, path: '/desk/settings' }
  ];

  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans">
      {/* Sidebar */}
      <aside className="w-64 bg-amber-50 border-r border-gray-100 flex flex-col shadow-sm z-10 transition-all duration-300">
        <div className="h-16 flex items-center px-6 border-b border-gray-200/50 bg-white/50 backdrop-blur-sm">
          <div className="flex items-center gap-3 w-full">
             <div className="p-1.5 bg-amber-50 text-amber-600 rounded-lg shadow-sm">
               <CalendarCheck size={22} strokeWidth={2.5} />
             </div>
             <span className="text-lg font-bold text-amber-600 tracking-tight">데스크</span>
          </div>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) => 
                `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                  isActive 
                  ? 'bg-white text-amber-600 font-bold shadow-md ring-1 ring-gray-100 scale-[1.02]' 
                  : 'text-gray-600 hover:bg-amber-100 hover:text-gray-900 font-medium'
                }`
              }
            >
              {item.icon}

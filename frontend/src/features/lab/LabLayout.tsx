...
const LabLayout = () => {
  ...
  const menuItems = [
    { name: '대시보드', icon: <LayoutDashboard size={20} />, path: '/lab' },
    { name: '환경설정', icon: <Settings size={20} />, path: '/lab/settings' }
  ];

  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans">
      {/* Sidebar */}
      <aside className="w-64 bg-indigo-50 border-r border-gray-100 flex flex-col shadow-sm z-10 transition-all duration-300">
        <div className="h-16 flex items-center px-6 border-b border-gray-200/50 bg-white/50 backdrop-blur-sm">
          <div className="flex items-center gap-3 w-full">
             ...
             <span className="text-lg font-bold text-indigo-600 tracking-tight">통합기공실</span>
          </div>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) => 
                `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                  isActive 
                  ? 'bg-white text-indigo-600 font-bold shadow-md ring-1 ring-gray-100 scale-[1.02]' 
